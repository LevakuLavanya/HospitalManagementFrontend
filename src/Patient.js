...
const PatientForm = () => {
  ...
  const fetchPatients = async () => {
    try {
      const response = await axios.get('http://localhost:5000/patients');  // Update endpoint URL
  ...
  const fetchDoctors = async () => {
    try {
      const response = await axios.get('http://localhost:5000/doctors');  // Update endpoint URL
  ...
  const handleSubmit = async (e) => {
    ...
    axios.post('http://localhost:5000/patients', updatedPatientData)
      .then(response => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/${id}`);  // Update endpoint URL
